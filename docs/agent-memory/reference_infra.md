...
- **Public URL**: `https://sso.weldforge.org`. Internal API health: `kubectl -n sso exec deploy/sso-api -c sso-api -- curl -s http://localhost:8080/actuator/health`. `/actuator/**` is **not** exposed through the public ingress — an external request falls through to the marketing SPA and returns HTML, so don't use it as an outside-in liveness check.
