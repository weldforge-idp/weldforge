    ...
    private Result dispatch(PaymentGatewayStrategy.NormalisedEvent event) {
        ...
        switch (event.type()) {
            case CHECKOUT_COMPLETED, PAYMENT_SUCCEEDED -> {
                ...
                if (order.getStatus() == tech.cwvermaak.weldforge.model.payment.OrderStatus.PROVISIONED
                        || order.getStatus() == tech.cwvermaak.weldforge.model.payment.OrderStatus.REFUNDED) {
